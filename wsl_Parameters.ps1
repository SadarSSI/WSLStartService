...
$Params = @{
  ...
  Task = @{
    User     = "$env:username"
    TaskName = "Wsl_"
    TaskAction        = 'New-ScheduledTaskAction -Execute Powershell.exe -Argument "-executionpolicy remotesigned -File ""$PSScriptRoot\wsl_services.ps1""" -WorkingDirectory "$PSScriptRoot" '
